 ...
 * Re-adapted for GLUT framework by Dave Milici, <davem@ironicresearch.com>.
 *
 */
...
void Display(void)
{
	...
	for (i=0; i<2; i++)
	{
		...
		if (bStereo)
                ...
                /* Depth buffer might be shared among draw buffers
                 * on some graphics systems, so clear for each eye.
                 * (Originally identified and noted by Akka.)
                 * Explicit stereo buffers must be cleared inidvidually
                 * on ATI Radeon GL drivers. (DaveM)
                 */
                if (bStereo)
                    glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
#endif
