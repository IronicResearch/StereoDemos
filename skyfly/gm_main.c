...
/* Stereo projection matrix calculation routine */
...
void draw(void)
{
	...
	if (bStereoEnabled) {
		...
		glPushMatrix();
		glLoadMatrixf((eye==0) ? &mProjectionLeft[0] : &mProjectionRight[0]);
...
keyboard(unsigned char c, int x, int y)
{
    ...
    case 't':
        ...
        break;
#ifdef 	STEREO	/* Adjust stereo parallax settings */
	case 's':
		bStereoEnabled = !bStereoEnabled;
		break;
	case '+':
		fDxViewpoint += 0.01;
		calcStereoMatrix();
...
#ifdef STEREO
  glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE | GLUT_STEREO | GLUT_DEPTH | GLUT_MULTISAMPLE);
#else
...
#ifdef STEREO
  /* Check if GL_STEREO quad-buffering enabled ? */
